	...
	<div class="bg-surface-100-900 max-w-[80vw] rounded-lg p-6 shadow">
		<div class="tabs">
			<button
				...
				class:active={activeTab === 'gamesettings'}
				on:click={() => (activeTab = 'gamesettings')}>Nustatymai</button
			>

			<button
				class="btn preset-filled-primary-500"
				class:active={activeTab === 'questions'}
				on:click={() => (activeTab = 'questions')}>Klausimai</button
			>
		</div>
		{#if activeTab === 'gamesettings'}
			<div class="mb-4 flex items-center justify-between">
				<h2 class="h5">Žaidimo nustatymai</h2>
